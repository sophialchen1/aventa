...
<template>
    <div-custom class="py-7 px-3 lg:px-0">
        <div class="w-full flex flex-col lg:gap-5">
            <subtitulo-custom :subtitulo="$t('ini_sub6')"/>
            <div v-for="s in instalaciones" class="w-full lg:h-[500px] flex flex-col gap-5 lg:gap-0 lg:flex-row">
                <div class="flex-1/4  lg:grid lg:grid-cols-1 lg:grid-rows-7 gap-5">
                    <div class="row-span-4 pt-8 flex flex-col gap-2">
                        <lista-ubicaciones :title="$t('loc_showroom')" :items="s.oficinas" :tipo="props.tipo" v-model:selected="seleccionado"/>
                        <lista-ubicaciones :title="$t('ini_p16')" :items="s.fabricas" :tipo="props.tipo" v-model:selected="seleccionado"/>
                    </div>
                    <div class="row-span-3 px-3 pt-3 hidden lg:flex">
                        <div class="flex flex-col justify-center items-center gap-2 w-full h-full shadow-md" :class="val ? 'bg-[#657d88]/10 rounded-xl' : 'bg-white'">
                            <h4 class="text-lg font-bold text-center text-[#657d88]">{{ $t("ini_p17") }}</h4>
                            <p class="text-center px-3">{{ val ? $t('ini_p18') : 'Agenda una visita privada' }}</p>
                            <btn-primary-normal v-if="val" :tipo="props.tipo" :titulo="$t('ini_btn5')" class="w-44" @click="openModal"/>
                ...
                <div class="flex-3/4 bg-gray-300 p-2 shadow-lg" :class="val ? 'rounded-xl' : 'bg-white'">
                    <div class="flex flex-col lg:grid lg:grid-cols-2 w-full h-full">
                        ...
                        <div class="flex flex-col gap-5 justify-center w-full h-full px-2">
                            ...
                            <p class="h-18">{{ seleccionado.i18n_dir ? $t(seleccionado.i18n_dir) : seleccionado.direccion }}</p>
                            <div class="flex items-center">
                                <a class="flex-1" :href="'Tel:+'+seleccionado.telefono" @click="window.gtag_report_conversion('Tel:+'+seleccionado.telefono, 'phone_click')" onclick="return gtag_report_conversion(undefined,'phone_click')" target="_blank"><p><b>{{ $t('loc_phone') }}</b> +{{ seleccionado.telefono }}</p></a>
                            ...
                            <div class="flex justify-center lg:justify-start">
                                <btn-primary-router v-if="val" :titulo="$t('f_visit')" link="/planea-visita" class="w-40"/>
                                <btn-primary-normal v-else :tipo="props.tipo" titulo="Ver Más" class="w-40"/>
                            </div>
                        </div>
                    </div>
